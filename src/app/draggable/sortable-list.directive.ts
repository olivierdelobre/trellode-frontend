...
export class SortableListDirective implements AfterContentInit {
  @ContentChildren(SortableDirective) sortables: QueryList<SortableDirective> = new QueryList<SortableDirective>();
  ...
  ngAfterContentInit(): void {
    this.sortables.forEach(sortable => {
      ...
      sortable.dragEnd.subscribe(event => this.notifyDragEnd(sortable, event));
    });
  }
  ...
  private notifyDragEnd(sortable: SortableDirective, event: PointerEvent) {
    this.dragEnded.emit({});
  }
}
